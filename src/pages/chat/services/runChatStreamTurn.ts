...
export async function runChatStreamTurn(opts: RunChatStreamTurnOptions): Promise<void> {
  ...
  if (isDeepResearch) {
    // Cost confirmation for expensive research depths (ultra4x, ultra8x).
    try {
      const { shouldConfirmCost, COST_ESTIMATES } = await import(
        "@/hooks/useCostConfirmation"
      );
      const { confirmCostGlobal } = await import(
        "@/components/billing/CostConfirmationHost"
      );
      if (shouldConfirmCost("research", researchDepth)) {
        const ok = await confirmCostGlobal({
          estimatedCost: COST_ESTIMATES.research[researchDepth] ?? 10,
          operation: `Deep Research — ${researchDepth}`,
          reason:
            "Deep research at this depth analyzes many sources and can take several minutes. It uses significantly more credits than a standard chat reply.",
          suppressKey: `research:${researchDepth}`,
        });
        if (!ok) {
          setIsLoading(false);
          setIsThinking(false);
      ...
      console.warn("[research] cost confirm failed", e);
    }
    try {
